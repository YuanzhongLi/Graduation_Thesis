...
def plot_movement(coordsG, node_id=23):
    n_t = coordsG.shape[0]
    x_data = np.zeros((n_t), dtype=np.float64)
    y_data = np.zeros((n_t), dtype=np.float64)
    z_data = np.zeros((n_t), dtype=np.float64)
    x_0 = np.zeros((n_t), dtype=np.float64)
    y_0 = np.zeros((n_t), dtype=np.float64)
    z_0 = np.zeros((n_t), dtype=np.float64)
    for i in range(n_t):
#         x_data[i] = coordsG[i, 1, node_id-1]
#         y_data[i] = coordsG[i, 2, node_id-1]        
#         z_data[i] = coordsG[i, 0, node_id-1]
#         x_0[i] = coordsG[i, 1, 0]
#         y_0[i] = coordsG[i, 2, 0]        
#         z_0[i] = coordsG[i, 0, 0]
        x_data[i] = coordsG[i, 0, node_id-1]
        y_data[i] = coordsG[i, 1, node_id-1]        
        z_data[i] = coordsG[i, 2, node_id-1]
        x_0[i] = coordsG[i, 0, 0]
        y_0[i] = coordsG[i, 2, 0]        
        z_0[i] = coordsG[i, 1, 0]
    
    max_range = np.array([x_data.max()-y_data.min(), y_data.max()-y_data.min(), z_data.max()-z_data.min()]).max() * 0.5
    ...
    for i in range(5):
        if i == 4:
            ax.plot(x_data[i*sec:],y_data[i*sec:],z_data[i*sec:], color=colors[i], marker='o')
            ax.plot(x_0[i*sec:],y_0[i*sec:],z_0[i*sec:], color=colors[i])                        
        else:
            ax.plot(x_data[i*sec:(i+1)*sec],y_data[i*sec:(i+1)*sec],z_data[i*sec:(i+1)*sec], color=colors[i], marker='o')            
            ax.plot(x_0[i*sec:(i+1)*sec],y_0[i*sec:(i+1)*sec],z_0[i*sec:(i+1)*sec], color=colors[i])                        
            
    plt.show()


plot_movement(coordsG)
