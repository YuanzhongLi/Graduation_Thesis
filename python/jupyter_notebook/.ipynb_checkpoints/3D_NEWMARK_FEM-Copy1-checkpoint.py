...
def dumping_3dfrm(gamma, omega, ek, mass):
    ek = omega * ek
    m = mass * gamma
    for i in range(0, 12):
        ek[i, i] += m
...
# dumpingのparam: ganma, omega
def newmark_3dfrm(delta_t, alpha, beta, gamma, omega, ek, mass):
    m = ((1.0+alpha+beta+gamma) / (beta*(delta_t**2))) * mass
    ret = (1.0 + ((alpha*omega) / (beta*delta_t))) * ek
    for i in range(0, 12):
        ret[i, i] += m
...
def main_3d_NEWMARK_FEM():
        ...
        acc=np.zeros((n_t+1, 6*npoin), dtype=np.float64)
        vec=np.zeros((n_t+1, 6*npoin), dtype=np.float64)
        dis=np.zeros((n_t+1, 6*npoin), dtype=np.float64)
        for step in range(1, n_t+1):
            ...
            for ne in range(0, nele):
                ...
                # c_matによる部分を計算
                tmp=np.zeros(nod*nfree, dtype=np.float64)
            ...
            for i in range (0, npoin):
                for j in range(0, nfree):
                    if mpfix[step, j, i] == 1:
                        iz=i*nfree+j
                        fp[step]=fp[step]-rdis[step, j, i]*gk[:,iz]
                        gk[:,iz]=0.0
                        gk[iz,iz]=1.0
            # 疎行列圧縮格納
            gk = csr_matrix(gk)            
            dis[step] = spsolve(gk, fp[step])
