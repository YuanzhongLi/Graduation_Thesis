...
#             return sorted(eig_val)
            ...
            dis[cur_32step] = spsolve(sp_newmark_mat, fp[step], use_umfpack=True)
#             dis[cur_32step] = np.linalg.solve(newmark_mat, fp[step])
...
check = main_3d_NEWMARK_FEM('test_verification1.txt')
...
with open('amp_vr2_等加速度.txt', 'w', encoding='ascii') as f:
    for i in range(300):
        step = i + 1
        f.write('{0:.3f} {1:.3f}\n'.format(step / 1000, 0.003))
