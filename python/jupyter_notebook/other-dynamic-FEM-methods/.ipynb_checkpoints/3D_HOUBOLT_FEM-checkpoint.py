...
# HOUBOLT マトリックスを作成
def houbolt_3dfrm(delta_t, gk, mass_mat, c_mat):
    for i in range(0, len(gk)):
        gk[i, i] += (2.0/(delta**2))*mass_mat[i, i]
    return gk + (11.0/(6*delta_t))*c_mat


def main_3d_HOUBOLT_FEM(file_path):
        start=time.time()
        args = sys.argv
        fnameR=args[1]
        fnameW=args[2]
        ...
        mass_mat, _ =mass_3dfrm(ae[6], npoin, nfree)
        ...
        for step in range(3, n_t+1):
            ...
            houbolt_mat=houbolt_3dfrm(delta_t, gk, mass_mat, c_mat)
            # fpを整理
            tmp1 = (3.0/delta_t)*c_mat
            tmp2 = (3.0/(2*delta_t))*c_mat
            tmp3 = (1.0/(3*delta_t))*c_mat
            for i in range(0, len(c_mat)):
                tmp1[i, i] += (5.0/(delta_t**2))*mass_mat[i, i]
                tmp2[i, i] += (4.0/(delta_t**2))*mass_mat[i, i]
                tmp3[i, i] += (1.0/(delta_t**2))*mass_mat[i, i]
            
            fp[step] = fp[step] + np.dot(tmp1, dis[step-1]) - np.dot(tmp2, dis[step-2]) + np.dot(tmp3, dis[step-3])
            ...
            for i in range (0, npoin):
                for j in range(0, nfree):
                    if mpfix[step, j, i] == 1:
                        iz=i*nfree+j
                        houbolt_mat[:,iz]=0.0
                        houbolt_mat[iz,iz]=1.0
            # 疎行列圧縮格納
            sp_houbolt_mat = csr_matrix(houbolt_mat)
            dis[step] = spsolve(sp_houbolt_mat, fp[step], use_umfpack=True)
            ...
            # 速度, 加速度計算            
            acc[step] = (1.0/(delta_t**2))*(2*dis[step] - 5*dis[step-1] + 4*dis[step-2] - dis[step-3])
            vec[step] = (1.0/(6*delta_t))*(11*dis[step] - 18*dis[step-1] + 9*dis[step-2] - 2*dis[step-3])        
        dtime=time.time()-start
        print('time: {0:.3f}'.format(dtime)+'sec')
        return dis


dis = main_3d_HOUBOLT_FEM('test001.txt')
